...
namespace GPU_VARIANZAS{
    ...
    __global__ void simularLognormales(double * resultados, int * divisores, double * varianzas, double ** muestras, double ** auxiliares) {
        int tid = blockIdx.x;
    ...
    //  double * auxiliar = new double[INDIVIDUOS_POR_SIMULACION];
        ...
        for (int s = 0; s < (2 * SIMULACIONES_POR_BLOQUE); ++s) {
            __syncthreads();
            double stddev = sqrt(varianzas[(index+s)/SIMULACIONES_POR_VARIANZA]);
            double * resultado = resultados + (index+s) * (NUM_DIVISORES+1);
            curandState state;
            curand_init(index + s, 0, threadIdx.x * SIZE, &state);

            if(threadIdx.x < 32){
                for (int i = threadIdx.x * SIZE; i < (threadIdx.x+1) * SIZE; ++i) {
                    muestra[i+1] = curand_log_normal(&state, 0.0f, stddev);
                }
            }
            __syncthreads();
            ...
            if(threadIdx.x == 0){
                resultado[0] = varianzas[index/SIMULACIONES_POR_VARIANZA];
            ...
            resultado[threadIdx.x+1] = suma/(INDIVIDUOS_POR_SIMULACION/divisores[threadIdx.x]);
            __syncthreads();
        }
    }
    ...
    __host__ void SimulacionCuda(){
        ...
        size_t resultSize = numSimulaciones * (1+NUM_DIVISORES) * sizeof(double);
        ...
        double * d_varianzas;
        cudaMalloc(&d_varianzas, NUM_VARIANZAS * sizeof(double));
        cudaMemcpy(d_varianzas, VARIANZAS, NUM_VARIANZAS * sizeof(double), cudaMemcpyHostToDevice);
        // Launch kernel
        simularLognormales<<<(BLOCK_COUNT / 2), NUM_DIVISORES>>>(d_resultados, d_divisores, d_varianzas, d_muestras, d_auxiliar);

        // Copy results from device to host
        cudaMemcpy(h_resultados, d_resultados, resultSize, cudaMemcpyDeviceToHost);

        int * titulos = (int *)malloc((NUM_DIVISORES+1)*sizeof(int));
        memcpy(titulos+1,DIVISORES,sizeof(DIVISORES));
        titulos[0] = -1;

        exportCSV(h_resultados, titulos, NUM_DIVISORES+1, (NUM_DIVISORES+1) * numSimulaciones, "datos_gpu_varianzas.csv");
