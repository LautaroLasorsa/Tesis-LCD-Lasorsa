...
def GenerarMuestra(datos_originales, umbral):
    muestras = dict()
    years = set(datos_originales["year"]).intersection(datos.columns)
    for year in sorted(years):
        #try:
            ...
            dato = datos[~ datos[str(year)].isna()]
            paises = set(paises).intersection(dato['Country Code'].unique())
            paises = list(paises) 
            ...
            dato.set_index('Country Code', inplace=True)
            ...
            for country in paises:
                ...
                muestras[year].append((country,muestra, dato.loc[country][str(year)]))
...
fig.suptitle('Correlación del ingreso con suscripciones de telefonía celular', fontsize=40)
...
xticks = list(range(1995,2022))
xlabel_ticks = list(map(str, xticks))

yticks_mayor = np.linspace(0.0,1,21)
yticks_menor = np.linspace(0.0,1,41)
...
for axi in range(len(axs)):
    ...
    ax.set_ylabel(f"{correlacion_nombre} con suscripciones moviles", fontsize=20)
...
fig.savefig("../figuras/figura_23_empirico_todas_vs_celulares.png",bbox_inches='tight')